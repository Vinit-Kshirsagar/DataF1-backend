...
MODEL = "gemini-1.5-flash"  # Free tier, fast, good quality
...
- Note one specific observation (e.g. strong braking zones, early throttle application)
...
async def generate_summary(
    ...
    """
    Generate a 1-3 line AI insight summary using Gemini Flash (free tier).
    Returns fallback string if API call fails — never propagates error.
    """
    if not settings.GEMINI_API_KEY:
        return f"Telemetry data loaded for {driver}. Summary generation is temporarily unavailable."

    prompt = _build_prompt(
        driver, team, metric, session, race, year, data_points, fastest_lap
    )
    ...
    try:
        ...
        logger.info(f"Gemini summary generated for {driver} {metric} ({len(summary)} chars)")
        return summary

    except Exception as e:
        logger.error(f"Gemini API failed for {driver} {metric}: {e}")
        return (
            f"Telemetry data loaded for {driver}. "
